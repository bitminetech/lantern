...
static size_t signed_block_encoded_size(const LanternSignedBlock *block) {
    ...
    size_t message_size = block_encoded_size(&block->message);
    if (message_size == 0) {
        return 0;
    }
...
static int storage_store_block_internal(
    ...
    if (root_override) {
        root = *root_override;
    } else if (lantern_hash_tree_root_block(&block->message, &root) != 0) {
        goto cleanup;
    }
    ...
    if (encoded_size == 0) {
        lantern_log_warn(
            ...
            block->message.body.legacy_plain_attestation_layout ? "true" : "false",
...
int lantern_storage_collect_blocks(
    ...
    for (size_t i = 0; i < root_count; ++i) {
        ...
        lantern_log_trace(
            ...
            root_hex,
            dest->message.slot,
            dest->message.body.attestations.length);
        free(data);
    }
    rc = 0;
