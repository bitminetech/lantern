...
static bool pending_contains_root(struct lantern_client *client, const LanternRoot *root)
{
    if (!client || !root)
    {
        return false;
    ...
    for (size_t i = 0; i < client->pending_blocks.length; ++i)
    {
        ...
        if (memcmp(entry->root.bytes, root->bytes, LANTERN_ROOT_SIZE) == 0
            || memcmp(entry->parent_root.bytes, root->bytes, LANTERN_ROOT_SIZE) == 0)
        {
            found = true;
            break;
        }
...
static bool should_request_missing_root(
    ...
    const char *peer_text)
{
    ...
    if (!vote_processed)
    {
        ...
        {
            request_missing_checkpoint_root(
                client,
                peer_text,
