...
static int diagnose_transition_failure(
    ...
    *out_hit_expected_fail_condition = false;
    ...
    if (load_snapshot_buggy(data_dir, &block->message.parent_root, &state) != 0) {
    ...
    int slots_rc = lantern_state_process_slots(&state, block->message.slot);
    ...
    int header_rc = lantern_state_process_block_header(&state, &block->message);
...
static int replay_target_block(
    ...
    if (root_to_hex(&block.message.parent_root, parent_hex, sizeof(parent_hex), true) != 0) {
    ...
    if (load_snapshot_buggy(data_dir, &block.message.parent_root, &buggy_state) != 0) {
    ...
    if (load_snapshot_fixed(data_dir, &block.message.parent_root, &fixed_state) != 0) {
